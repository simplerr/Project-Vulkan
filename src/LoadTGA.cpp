...
namespace VulkanLib
{
	...
	bool LoadTGATextureData(char *filename, TextureData *texture)	// Loads A TGA File Into Memory
	{
		...
		texture->texWidth = (float)texture->width / w;
		texture->texHeight = (float)texture->height / h;
	...
	/*
	void LoadTGATextureSimple(char *filename, int *tex) // If you really only need the texture object.
	{
		TextureData texture;
